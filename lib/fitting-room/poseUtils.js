  ...
  /**
   * MediaPipe Pose Landmarker (BlazePose, 33-point topology) keypoint indices.
   * Reference: https://ai.google.dev/edge/mediapipe/solutions/vision/pose_landmarker
   *
   * NOTE: `score` throughout this file is populated from MediaPipe's
   * `visibility` field by the estimatePoses() adapter in
   * FittingRoomProvider.jsx — it is not a native MoveNet-style field anymore,
   * just kept under the same name so analyzePose()/detectBodyShapeFromPose()
   * and ScanPanel's detect loop need no changes.
   *
   * PENDING: HIP_KP_CORRECTION, BODY_DEPTH, and SEGMENT_SCALE below were
   * tuned against MoveNet's joint placement. BlazePose places landmarks
   * slightly differently, so these need a recalibration pass against tape
   * measurements before trusting the output — tracked as a follow-up, not
   * part of this swap.
   */
  export const KP = {
    NOSE: 0,
