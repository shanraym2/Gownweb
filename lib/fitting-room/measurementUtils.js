...
// ─────────────────────────────────────────────────────────────────────────────
// estimateEllipseCircumferenceMeasured(widthCm, depthCm)
//
// Same Ramanujan ellipse approximation as estimateEllipseCircumference(),
// but takes an ACTUAL measured depth in cm (from a side-scan silhouette
// reading) instead of a guessed depthRatio from the BODY_DEPTH table. This
// is the whole point of the side scan — it replaces a per-shape guess with
// a real front-to-back measurement of THIS subject.
//
// @param {number} widthCm  Lateral body width in cm (front scan)
// @param {number} depthCm  Front-to-back body depth in cm (side scan)
// @returns {number}        Estimated circumference in cm
// ─────────────────────────────────────────────────────────────────────────────

export function estimateEllipseCircumferenceMeasured(widthCm, depthCm) {
  const a = widthCm / 2
  const b = depthCm / 2
  return Math.PI * Math.sqrt(2 * (a * a + b * b))
}
...
// ─────────────────────────────────────────────────────────────────────────────
// estimateMeasurementsWithDepth({ shoulderCm, waistCm, hipCm,
//                                  bustDepthCm, waistDepthCm, hipDepthCm,
//                                  bodyShape, segment })
//
// Side-scan variant of estimateMeasurements(). Uses a MEASURED depth in cm
// per region when available (from the side-scan silhouette reader) instead
// of the BODY_DEPTH shape-table guess. Falls back to the guessed ratio for
// any region whose depth measurement is missing — e.g. if the side scan
// got a usable waist reading but the hip row was occluded by an arm, only
// the waist estimate benefits from the real depth in this call.
//
// SEGMENT_SCALE corrections are still applied — those compensate for the
// keypoint-to-silhouette WIDTH offset (shoulders/hips extend beyond the
// joint centres MoveNet/BlazePose mark), a separate issue from depth that
// applies whether depth is measured or guessed. They have not been
// re-validated against measured-depth output yet — flagged as a follow-up
// once real side-scan tape-measure data is available for comparison.
//
// @returns {{ bust:number, waist:number, hips:number }}
// ─────────────────────────────────────────────────────────────────────────────

export function estimateMeasurementsWithDepth({
  ...
  // Measured depth wins when present; guessed ratio is the fallback per region
  const rawBust  = estimateEllipseCircumferenceMeasured(shoulderCm, bustDepthCm  ?? shoulderCm * depth.bust)
  const rawWaist = estimateEllipseCircumferenceMeasured(waistCm,    waistDepthCm ?? waistCm    * depth.waist)
  const rawHips  = estimateEllipseCircumferenceMeasured(hipCm,      hipDepthCm   ?? hipCm       * depth.hips)
